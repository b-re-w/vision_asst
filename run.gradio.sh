...
SERVER_PORT="${VA_PORT:-8070}"
GRADIO_PORT=8022
STAGE_CONFIGS_PATH=""
SERVER_HOST="${VA_HOST:-0.0.0.0}"
GRADIO_IP="${VA_HOST:-0.0.0.0}"
GRADIO_SHARE=false
...
SERVER_CMD=("uv" "run" "vllm" "serve" "$MODEL" "--omni" "--port" "$SERVER_PORT" "--host" "$SERVER_HOST")
...
cd "$SCRIPT_DIR"
GRADIO_CMD=("uv" "run" "./src/vision_client/web_client.py" "--model" "$MODEL" "--api-base" "$API_BASE" "--ip" "$GRADIO_IP" "--port" "$GRADIO_PORT")
